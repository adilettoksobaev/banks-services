...
const DeleteDepartment:React.FC<Props> = ({ deleteOpen, deleteClickClose, title, text, id, sessionId, positionSuccessAction, positionSuccess }) => {
    ...
    const deleteEmployee = () => {
        if(sessionId) {
            AdminAPI.deleteDepartment(sessionId, id).then(data => {
                if(data.success === true) {
                    setNotification(true);
                    setErrorMessage({error: false, message: 'Вы успешно удалили отдел!'});
                    deleteClickClose();
